...
from automata_toolkit.consts import Consts
import uuid
...
def dfa_to_regex(dfa):
    ...
    gnfa = {}
    gnfa["initial_state"] = str(uuid.uuid4())
    gnfa["final_states"] = [str(uuid.uuid4())]
    ...
    gnfa["transition_function"][gnfa["initial_state"]] = {
        Consts.EPSILON: dfa["initial_state"]}
    ...
    reachable_non_dead_states = filter(
        lambda x: is_not_dead[x], dfa["reachable_states"])
    ...
    for chosen_state in reachable_non_dead_states:
        ...
        if string != "":
            ...
            next_states = list(
                gnfa["transition_function"][chosen_state].items())
            for alphabet, next_state in next_states:
                ...
                if chosen_state != next_state:
                    for ind in range(len(L[chosen_state][next_state])):
                        L[chosen_state][next_state][ind] = concat_regex(
                            string, L[chosen_state][next_state][ind])
                        gnfa["transition_function"][chosen_state][L[chosen_state]
                                                                  [next_state][ind]] = next_state
        ...
        for prev_state in gnfa["states"]:
            if prev_state != chosen_state:
                prev_next_states = list(
                    gnfa["transition_function"][prev_state].items())
