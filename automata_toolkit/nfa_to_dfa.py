...
def get_epsilon_closure(nfa, dfa_states, state):
    ...
    while len(state_stack) > 0:
        ...
        if nfa["transition_function"][current_state][alphabet] not in closure_states:
            state_stack.extend(nfa["transition_function"]
                               [current_state][alphabet])
...
def nfa_to_dfa(nfa):
    ...
    # dfa["alphabets"] = ["a", "b"]
    dfa["alphabets"] = list(
        filter(lambda x: x != Consts.EPSILON, nfa["alphabets"]))
            ...
            if state == "phi":
                ...
                if len(state) == 1:
                    ...
                    for next_nfa_state in next_nfa_states:
                        transition_states.extend(
                            epsilon_closure[next_nfa_state])
                else:
                    for nfa_state in state:
                        nfa_state = tuple([nfa_state])
                        if dfa["transition_function"][nfa_state][alphabet] != "phi":
                            transition_states.extend(
                                dfa["transition_function"][nfa_state][alphabet])
                ...
                if len(transition_states) == 0:
                    ...
                    dfa["transition_function"][state][alphabet] = find_permutation(
                        dfa["states"], transition_states)
    ...
    dfa["final_reachable_states"] = list(
        set(dfa["final_states"]) & set(dfa["reachable_states"]))
